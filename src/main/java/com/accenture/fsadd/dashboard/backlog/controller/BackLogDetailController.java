...
public class BackLogDetailController {
	...
	public ApiModel<BackLogDetailModel> getQaDetailAction() {
		...
		openedBackLogList.forEach((openBackLog) -> {
			...
			backLogEntryModel.setDiffHours(openBackLog.getActualHours() - openBackLog.getEstimatedHours());
	...
	private void setDailyBurnDownDetail(BackLogDetailModel backLogDetailModel) {
		...
		for (int i = 0; i <= BACK_DAY_COUNT; i++) {
			...
			if (backLogDailySummaryEntity != null) {
				backLogDailyBurnDownModel.getPlanCompletedCount()
						.add(backLogDailySummaryEntity.getClosedCount() + backLogDailySummaryEntity.getDelayCount());
				backLogDailyBurnDownModel.getActualCompletedCount().add(backLogDailySummaryEntity.getClosedCount());
			} else {
