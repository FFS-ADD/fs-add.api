...
public class DefaultRedmineDataMapper implements ExtIfDataMapper {
  ...
  public void map(LocalDateTime lastExecutedTime) {
    ...
    List<BasicDBObject> issueList = mongoTemplate.findAll(BasicDBObject.class, redmineSetting.getIssueCollectionName());
    ...
    for (BasicDBObject dbObject : issueList) {
      ...
      // QA
      if (redmineSetting.getQaSetting() != null && redmineSetting.getQaSetting().getTrackerId().contains(trackerId)) {
        mapToQA(lastExecutedTime, redmineSetting, dbObject);
      }
      // Issue
      if (redmineSetting.getIssueSetting() != null
          && redmineSetting.getIssueSetting().getTrackerId().contains(trackerId)) {
        mapToIssue(lastExecutedTime, redmineSetting, dbObject);
      }
      // BackLog
      if (redmineSetting.getBackLogSetting() != null
          && redmineSetting.getBackLogSetting().getTrackerId().contains(trackerId)) {
        mapToBackLog(lastExecutedTime, redmineSetting, dbObject, backLogIds);
      }
      // Task
      if (redmineSetting.getTaskSetting() != null
          && redmineSetting.getTaskSetting().getTrackerId().containsKey(trackerId)) {
   ...
   * @param redmineSetting Redmine Setting
  ...
  protected void mapToQA(LocalDateTime lastExecutedTime, RedmineSetting redmineSetting, BasicDBObject dbObject) {
    ...
    if (queryAnswerEntity.getStatus() == QueryAnswerEntity.Status.CLOSED && queryAnswerEntity.getClosedOn() == null) {
  ...
  protected void mapToIssue(LocalDateTime lastExecutedTime, RedmineSetting redmineSetting, BasicDBObject dbObject) {
    ...
    for (BasicDBObject customField : customFields) {
      ...
      if (id.equals(redmineSetting.getIssueSetting().getCustomIdPhase())) {
        issueEntity.setPhase(redmineSetting.getIssueSetting().getPhaseMappingSetting()
            .get(customField.getString("value")));
      }
    }
   ...
   * @param redmineSetting Redmine Setting
   * @param dbObject Db Object
   * @param backLogIds BackLog Id set
   */
  @SuppressWarnings("unchecked")
  protected void mapToBackLog(LocalDateTime lastExecutedTime, RedmineSetting redmineSetting, BasicDBObject dbObject,
    ...
    Integer statusId = getNestedDbObjectInteger((BasicDBObject) dbObject.get("status"), "id");
    String estimatedHours = dbObject.getString("estimated_hours");
    if (estimatedHours != null) {
      backLogEntity.setEstimatedHours(Double.valueOf(dbObject.getString("estimated_hours")).intValue());
    }
    ...
    for (BasicDBObject customField : customFields) {
      ...
      if (backLogEntity.getEstimatedHours() == 0
          && id.equals(redmineSetting.getBackLogSetting().getCustomIdEstimatedHours())) {
        try {
          backLogEntity.setEstimatedHours(Double.valueOf(customField.getString("value")).intValue());
      ...
      if (id.equals(redmineSetting.getBackLogSetting().getCustomIdActualHours())) {
        try {
          backLogEntity.setActualHours(Double.valueOf(customField.getString("value")).intValue());
        } catch (NumberFormatException e) {
   ...
   * 
   * @param trackerId Tacker ID
   * @param lastExecutedTime time that last extif was executed.
   * @param redmineSetting Redmine Setting
   * @param dbObject Db Object
   * @param parentChildTaskIds Parent -> Child Task Id Map;
   * @param childParentIds Child -> Parent Task Id Map;
   */
  @SuppressWarnings("unchecked")
  protected void mapToTask(Integer trackerId, LocalDateTime lastExecutedTime, RedmineSetting redmineSetting,
    ...
    if (!StringUtils.isEmpty(estimatedHours)) {
      taskEntity.setEstimatedHours(Double.valueOf(dbObject.getString("estimated_hours")).intValue());
    }
    ...
    for (BasicDBObject customField : customFields) {
      ...
      if (taskEntity.getEstimatedHours() == 0 && id.equals(redmineSetting.getTaskSetting().getCustomIdEstimatedHours())) {
        try {
          taskEntity.setEstimatedHours(Double.valueOf(customField.getString("value")).intValue());
        } catch (NumberFormatException e) {
      ...
      if (id.equals(redmineSetting.getTaskSetting().getCustomIdActualHours())) {
        try {
          taskEntity.setActualHours(Double.valueOf(customField.getString("value")).intValue());
        } catch (NumberFormatException e) {
